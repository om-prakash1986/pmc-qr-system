...
/// <summary>
/// Summary description for ARV_for_Land_and_building
/// </summary>
public class ARV_for_Land_and_building
{
    ARV_for_vaccant_land ar = new ARV_for_vaccant_land();
    // PRIME ARV CALCULATION
    public string calculate_ARV_house_and_building(string total_area, string builtup_area, string use_type, string road_type, string construction_type, string occupancy_type, string usage_type, string municipality_type, string pid, string ass_year)
    {
        string amount_1 = "0";

        string occupancy_factor = calculate_occupancy_factor(occupancy_type, ass_year);
        //string carpet_area = calculate_carpet_area(builtup_area, occupancy_factor);
        ...
        string carpet_area = calculate_carpet_area_new(builtup_area, use_type);
        ...
        if (ass_year == finyear)
        {
            if (currentMonth > 3 && currentMonth < 7)
            {
                ...
                amount_1 = Property_tax.ToString();
            }
